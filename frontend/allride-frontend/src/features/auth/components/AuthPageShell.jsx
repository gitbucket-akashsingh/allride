...
export function AuthPageShell({ backTo = "/", backLabel = "Back to Home", maxWidth = "max-w-md", children }) {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-black dark:text-white overflow-hidden relative flex items-center justify-center px-4 py-10">
      ...
      <Link
        to={backTo}
        className="fixed top-6 left-6 z-20 flex items-center gap-2 px-5 py-3 rounded-2xl border border-zinc-200 bg-white/80 backdrop-blur-xl hover:bg-white dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10 transition-all text-sm font-semibold text-zinc-700 dark:text-white"
      >
        <ArrowLeft size={18} />
        {backLabel}
      </Link>
      ...
      <button
        ...
        className="fixed top-6 right-6 z-20 p-3 rounded-2xl border border-zinc-200 bg-white/80 backdrop-blur-xl hover:bg-white dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10 transition-all text-zinc-700 dark:text-white"
      >
