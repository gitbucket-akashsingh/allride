function PhoneDeviceFrame({ children, className = "" }) {
    return (
        <div
        className={`group relative flex justify-center lg:justify-end ${className}`}
        style={{ perspective: "1200px" }}
      >
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] sm:w-[360px] h-[580px] rounded-full blur-3xl opacity-30 group-hover:opacity-50 transition-opacity duration-500 pointer-events-none"
          style={{
            background:
      ...
      <div className="phone-float relative">
        {/* Layer 2: tilt only */}
        <div
          className="phone-tilt relative transform-gpu"
           ...
           <div className="absolute -left-[2px] top-44 w-[3px] h-14 rounded-full bg-zinc-700/80" />
              <div className="absolute -right-[2px] top-36 w-[3px] h-16 rounded-full bg-zinc-700/80" />
                 ...
                 <div className="relative h-full rounded-[38px] overflow-hidden bg-black ring-1 ring-white/10">
                    <div
                     className="absolute top-3 left-1/2 -translate-x-1/2 z-30 h-[26px] w-[100px] rounded-full bg-black border border-white/10 shadow-inner"
                    />
  
              {/* Screen — fills bezel, no aspect ratio */}
